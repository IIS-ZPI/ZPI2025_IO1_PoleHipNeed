...
def _network_available() -> bool:
    """Checks if api.nbp.pl is reachable."""
    import socket
    try:
...
_SKIP_REASON = "Missing INTEGRATION_TESTS=1 flag or connection to api.nbp.pl"
...
# ---------------------------------------------------------------------------
# INT-08: fetch_quotes — real data from API
# ---------------------------------------------------------------------------

@require_integration
class TestFetchQuotesRealAPI(unittest.TestCase):
    """
    INT-08: NBP API contract verification.

    Checks that the API response structure has not changed:
    date formats, rate types (Decimal), data range.
    """
...
class TestRealQuotesCountSessionsInvariant(unittest.TestCase):
    ...
    def test_count_invariant_usd(self):
        """USD: sum of sessions == len(quotes) - 1."""
        quotes = fetch_quotes("USD", self.start, self.end)
    ...
    def test_count_invariant_eur(self):
        """EUR: sum of sessions == len(quotes) - 1."""
        quotes = fetch_quotes("EUR", self.start, self.end)
        rises, falls, unchanged = count_sessions(quotes)
        self.assertEqual(rises + falls + unchanged, len(quotes) - 1)

    def test_counts_are_non_negative(self):
        """No session counter can be negative."""
        quotes = fetch_quotes("USD", self.start, self.end)
        rises, falls, unchanged = count_sessions(quotes)
    ...
    def test_statistical_measures_on_real_data(self):
        """Statistical calculations on real NBP data should not raise exceptions."""
        quotes = fetch_quotes("USD", self.start, self.end)
    ...
    def test_session_analysis_on_real_data(self):
        """calculate_sessions on real data — sum == number of quotes."""
        quotes = fetch_quotes("EUR", self.start, self.end)
        sessions = [float(q.mid) for q in quotes]
...
# ---------------------------------------------------------------------------
# INT-10: Real data (two currencies) -> calculate_change_distribution
# ---------------------------------------------------------------------------

@require_integration
class TestTwoRealCurrenciesChangeDistribution(unittest.TestCase):
    """
    INT-10: Two currencies from real API -> calculate_change_distribution.

    Verifies a real production issue: two currencies can have
    a different number of quotes in the same period (e.g. national holidays),
    which causes an error in calculate_change_distribution.
    """
    ...
    def test_same_quote_count_produces_distribution(self):
        """
        If USD and EUR have the same number of quotes,
        calculate_change_distribution should return the correct result.
        """
        ...
        if len(sessions_usd) != len(sessions_eur):
            self.skipTest(
                f"USD ({len(sessions_usd)}) and EUR ({len(sessions_eur)}) "
                f"have a different number of quotes in this period"
            )
    ...
    def test_distribution_bucket_sum_invariant(self):
        """
        Sum of distribution buckets == number of quote pairs (len - 1).
        Test for USD vs EUR when they have an equal number of sessions.
        """
        quotes_usd = fetch_quotes("USD", self.start, self.end)
        quotes_eur = fetch_quotes("EUR", self.start, self.end)
        ...
        if len(sessions_usd) != len(sessions_eur):
            self.skipTest("Different number of quotes — skipped")
